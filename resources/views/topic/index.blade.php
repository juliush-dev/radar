        ...
        <div class="flex flex-col gap-8 h-full">
            ...
            <div class="absolute bottom-0 right-0 p-8">
                <x-splade-dropdown>
                    ...
                    <div
                        class="whitespace-nowrap py-5 px-5 rounded-sm bg-slate-900/95 shadow-sm shadow-emerald-300 flex flex-col gap-2 w-64">
            ...
            @if ($contributedSkills->count() == 0 && $contributedTopics->count() == 0 && $contributedLearningMaterials->count() == 0)
                <div class="h-full flex items.center justify-center">
                    <div
                        class="my-auto w-1/2 text-xl flex gap-5 items.center p-5 border border-slate-500/60 rounded-sm shadow shadow-emerald-800">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-11 h-11 my-auto text-green-200">
                            <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @if ($contributedSkills->count() > 0)
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        @foreach ($contributedSkills as $skill)
                            <div
                                class="flex flex-col gap-3 rounded border border-emerald-900 hover:shadow hover:shadow-emerald-500 hover:bg-green-700/5  p-3 transition duration-300">
                                <h3 class="text-md flex gap-2 text-yellow-300/50 group-hover:text-teal-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M15.362 5.214A8.252 8.252 0 0112 21 8.25 8.25 0 016.038 7.048 8.287 8.287 0 009 9.6a8.983 8.983 0 013.361-6.867 8.21 8.21 0 003 2.48z" />
                                ...
                                <div class="flex flex-col gap-3 py-2 px-1 text-slate-500">
                                    <div class="flex gap-2 items.center">
                                        ...
                                        <div class="flex gap-2 items.center my-auto">
                                            <span
                                                class="my-auto px-2 py-1 text-xs rounded bg-green-900/60 text-teal-600">{{ $skill->topic_group_covering_it }}</span>
                                        </div>
                                    </div>
                                    <div class="flex gap-2 items.center">
                                        <div class="flex gap-2 items.center">
                                        ...
                                        <div class="flex gap-2 items.center my-auto">
                                            ...
                                            @foreach ($yearLevels as $yearLevel)
                                                <span
                                                    class="my-auto px-2 py-1 text-xs rounded bg-green-900/60 text-teal-600">{{ $yearLevel }}</span>
                                            @endforeach
                                        </div>
                                    </div>
                                    <div class="flex gap-2 items.center">
                                        ...
                                        <div class="flex gap-2 items.center my-auto">
                                            ...
                                            @foreach ($fields as $field)
                                                <span
                                                    class="my-auto px-2 py-1 text-xs rounded bg-green-900/60 text-teal-600">{{ $field }}</span>
                                            @endforeach
                                        </div>
                                    </div>
