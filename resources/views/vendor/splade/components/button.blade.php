...
<SpladeButton {{ $attributes->only(['v-bind:spinner', ':spinner']) }}>
    <template #default="button">
        <{{ $type === 'link' ? 'Link' : 'button' }} :disabled="button.spinner"
            {{ $attributes->class([
                    'border border-amber-300 outline outline-1 p-2 text-sm inline-flex items.center justify.center gap-2 first-letter:capitalize leading-5 focus:text-amber-400 hover:text-amber-400 shadow-md shadow-amber-500/60 transition duration-500 ease-in-out',
                    ' hover:bg-amber-700 text-amber-400 border-0 hover:text-amber-200 focus:ring-amber-400' =>
