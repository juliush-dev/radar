<x-splade-component is="modal-wrapper" :base-attributes="$attributes->except('class')" :key="$modalKey" :close-button="$closeButton" :close-explicitly="$closeExplicitly"
    :name="$name">
    ...
    <div class="fixed inset-0 z-40 overflow-y-auto">
        ...
        <div class="flex min-h-full justify-center"
            ...
            <x-splade-component is="transition" child animation="fade" after-leave="modal.emitClose" animate="modal.animate"
                v-bind:class="{
                    ...
                    <div {{ $attributes->class('bg-slate-100/80 rounded relative') }}>
                        <div v-if="modal.closeButton" class="absolute top-0 right-0 pt-3 pr-3 z-10">
                            <button dusk="close-modal-button" @click="modal.close" type="button"
                                class="text-slate-100 hover:text-slate-100 focus:border-none focus:outline-none">
