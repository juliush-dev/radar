<x-splade-component is="button-with-dropdown" dusk="filters-dropdown">
    <x-slot:button>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-fuchsia-500" viewBox="0 0 20 20" fill="currentColor"
            :class="{
                'text-fuchsia-500': !@js($table->hasFiltersEnabled()),
                'text-fuchsia-500': @js($table->hasFiltersEnabled()),
            }">
    ...
    <div role="menu" aria-orientation="horizontal" aria-labelledby="filter-menu">
        @foreach ($table->filters() as $filter)
            <div>
                <h3 class="text-xs uppercase tracking-wide bg-fuchsia-100 p-3">
                    {{ $filter->label }}
                </h3>
                ...
                <div class="p-2">
                    @if ($filter->type === 'select')
                        <select name="filter-{{ $filter->key }}"
                            class="block focus:ring-fuchsia-500 focus:border-fuchsia-500 w-full shadow-sm text-sm border-fuchsia-500"
