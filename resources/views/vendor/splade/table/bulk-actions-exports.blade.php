<x-splade-component is="button-with-dropdown" dusk="bulk-actions-exports-dropdown" v-bind:close-on-click="true">
    ...
    <div class="min-w-max rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
        <div class="flex flex-col">
            <h3 v-if="table.hasSelectedItems" class="text-xs uppercase tracking-wide bg-green-100 px-4 py-2 border-b">
            ...
            @foreach ($table->getBulkActions() as $bulkAction)
                <button v-if="table.hasSelectedItems"
                    class="text-left w-full px-4 py-2 text-sm text-green-700 hover:bg-green-50 hover:text-green-900 font-normal"
            ...
            @if ($table->hasExports())
                <h3 class="text-xs uppercase tracking-wide bg-green-100 px-4 py-2 border-b">
                    {{ __('Export results') }}
                </h3>
            @endif
