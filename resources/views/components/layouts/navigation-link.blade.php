    ...
    <div class="flex flex-col gap-2">
            ...
            <x-nav-link :active="$isActive" :modal="$openAs == 'modal'" :slideover="$openAs == 'slideover'" :method="$method" :href="$route"
                ...
                @if (strlen($iconPath) > 0)
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="my-auto w-5 h-5 {{ $isActive ? 'text-amber-400' : '' }}">
            ...
            @if (strlen($slot) > 0)
                <div>
                    <svg @click.prevent="toggle" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        ...
                        class="w-6 h-6 my-auto cursor-pointer transition-all duration-300 {{ $isActive ? 'text-amber-400' : '' }}"
        ...
        @if (strlen($slot) > 0)
            <x-splade-transition enter='duration-300' show="toggled">
                <div class="flex flex-col gap-2 border-l border-amber-300 pl-4 ml-3 pt-4 border-dashed">
                    {{ $slot }}
                </div>
            </x-splade-transition>
        @endif
