<x-layouts.main-content>
    <div class="flex gap-6 justify-between shadow p-6 xl:px-20">
        <div class="flex gap-6 items-center">
            ...
            @php
                $isViewingTopics = Route::current()->getName() == 'topics.index' || Route::current()->getName() == 'topics.show';
                $isViewingSkills = Route::current()->getName() == 'skills.index' || Route::current()->getName() == 'skills.show' || Route::current()->getName() == 'skills.topics.show';
            @endphp
            @if ($isViewingTopics || $isViewingSkills)
                <div class="border rounded-full flex flex-nowrap gap-0 items-center -mb-1">
                    <Link href="{{ route('skills.index') }}"
                        class="px-3 py-0.5 rounded-l-full text-center first-letter:uppercase {{ $isViewingTopics ? 'bg-slate-400 text-slate-50' : 'bg-teal-500 text-white' }}">
                    View skills
                    </Link>
                    <Link href="{{ route('topics.index') }}"
                        class="w-fit px-3 py-0.5 rounded-r-full text-cente first-letter:uppercase {{ $isViewingSkills ? 'bg-slate-400 text-slate-50' : 'bg-teal-500 text-white' }}">
                    View topics only
                    </Link>
                </div>
            @endif
        ...
        <div class="flex gap-6 items-center">
            ...
            @auth
                ...
                <x-layouts.navigation-link resource="contributions" action="index" label="My contributions"
