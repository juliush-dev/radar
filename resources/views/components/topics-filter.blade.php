<x-splade-form method="get" preserve-scroll submit-on-change="reset" :default="[
    ...
    <x-splade-toggle class="w-full">
        <div class="flex gap-6 md:justify-end flex-wrap items-center overflow-x-auto">
            ...
            @endcan
            @can('create-topic')
                <Link title="Create new topic" href="{{ Auth::check() ? route('topics.create') : '#login-required' }}"
                    class="text-violet-500 hover:text-violet-600 whitespace-nowrap">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            ...
            <button @click.prevent="toggle">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-5 h-5 text-violet-500 hover:text-violet-600">
                    <path stroke-linecap="round" stroke-linejoin="round"
