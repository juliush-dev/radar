<x-splade-toggle data="uploadFormOpened, lmsOpened">
    <div v-bind:class="(uploadFormOpened == true ||lmsOpened == true) && 'bg-sky-400 border-none'"
        class="w-full break-inside-avoid border overflow-hidden flex flex-col border-slate-300 relative  text-teal-900 topic transition-all duration-100">
        <div class="flex flex-col gap-10 w-full grow justify-center items-center p-6 pt-8 transition-all duration-500">
            ...
            <div class="space-y-3 my-auto w-full">
                ...
                @endif
                <p class="font-mono break-all p-2 uppercase text-center text-xl group-hover:text-teal-800">
                    {{ $topic->title }}
                </p>
                @if ($topic->subject)
            ...
            <div class="flex gap-20 items-center transition-all duration-700">
                <button v-show="lmsOpened == false" @click.prevent="toggle('lmsOpened')" class="flex gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                ...
                <button v-show="lmsOpened == true" @click.prevent="toggle('lmsOpened')">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                ...
                <button v-show="uploadFormOpened == false" @click.prevent="toggle('uploadFormOpened')">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-6 h-6 mx-auto mb-2">
                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <button v-show="uploadFormOpened == true" @click.prevent="toggle('uploadFormOpened')" class="">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M9.75 9.75l4.5 4.5m0-4.5l-4.5 4.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            ...
                            <x-splade-form method="get" :action="route('topics.learning-materials.download', $lm->id)" blob
                                ...
                                @auth
                                    <x-splade-form :action="route('topics.learning-materials.remove', $lm->id)" class="absolute top-4 right-6">
                                        <button type="submit"
                                            class="flex gap-2 text-white text-sm rounded-full p-2 bg-red-600 rounded-t-none">
                                        @endauth
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
