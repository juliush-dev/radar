 <div
     ...
     <div class="font-normal text-sm mb-6 text-slate-500 dark:text-slate-300 flex items-center gap-3 flex-wrap">
         ...
         <span class="dark:text-slate-300 text-slate-500 flex items-center gap-1 flex-nowrap">
             {{ $topic->notes()->count() }}
             @if (($volatiles = $topic->volatileNotes()->count()) > 0)
                 <span class="px-2 rounded-full bg-pink-500 text-white text-xs my-auto">
                     % {{ $volatiles }}
                 </span>
             @endif
                 ...
                 @if ($topic->is_update && $topic->potentialReplacement == null)
                     @can('use-dashboard')
                         <x-splade-link method="post" :href="route('topics.apply-update', $topic)"
                             ...
                             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                 stroke="currentColor"
                                 class="w-5 h-5 text-teal-500 hover:text-teal-600 shadow
