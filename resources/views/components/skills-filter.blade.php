<x-splade-form method="get" preserve-scroll submit-on-change="reset" :default="[
    ...
    <x-splade-toggle class="w-full overflow-hidden">
        <div class="flex gap-6 md:justify-end flex-wrap items-center overflow-x-auto">
            ...
            @can('create-skill')
                <Link href="{{ Auth::check() ? route('skills.create') : '#login-required' }}"
                    class="text-sky-500 hover:text-sky-600 whitespace-nowrap">
            ...
            @endcan
            <button class="whitespace-nowrap flex gap-2 items-center" @click.prevent="toggle">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-5 h-5 text-sky-500 hover:text-sky-600">
            ...
            <button class="whitespace-nowrap flex gap-2 items-center" title="reset filter"
        ...
        <x-splade-transition show="toggled"
            ...
            <x-splade-select label="Where year is" class="w-full lg:w-max lg:grow" placeholder="..." name="year"
                :options="$years" option-value="id" option-label="title" />
            <x-splade-select label="Where field is" class="w-full lg:w-max lg:grow" placeholder="..." name="field"
