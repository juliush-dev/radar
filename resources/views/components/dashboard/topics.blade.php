<div>
    <x-splade-table :for="$topics">
        ...
        @cell('public', $topic)
            ...
            @endif
        @endcell
        @cell('topicToUpdate.title', $topic)
            @if ($topic->topicToUpdate == null)
                -
            @else
        ...
        @endcell
        @cell('action', $topic)
            {{-- <div class="w-full pr-4 overflow-x-auto h-full py-2"> --}}
            ...
            @if ($topic->topicUpdating == null)
                <x-splade-link :href="route('topics.edit', $topic)" class="mr-4">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor"
                        class="w-6 h-6 text-violet-500 hover:text-violet-600 shadow
                hover:shadow-md transition-all duration-300">
                        <path stroke-linecap="round" stroke-linejoin="round"
