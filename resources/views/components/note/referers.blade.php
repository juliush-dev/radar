@php
    $referers = $note->relativeOf
        ...
        ->all();
@endphp
<section v-if="@js(count($referers) > 0)">
    <h3 class="text-lg font-semibold">Referenced by</h3>
    <ul class="flex gap-4 flex-wrap mt-3">
        @foreach ($referers as $referer)
            <li class="w-fit  text-slate-500">
                <Link href="{{ route('topics.references', $note) }}" class="flex gap-2">
