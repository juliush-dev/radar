 <x-layouts.app :active-page="'Note / ' . $note->extractTitle()"
     ...
     <div class="h-full w-full flex flex-wrap lg:flex-nowrap gap-0 overflow-hidden">
         <section
             class="relative min-h-full overflow-y-auto min-w-[100%] lg:min-w-[50%] lg:border-r-4 border-slate-200 dark:border-slate-700">
             <div
         ...
         @foreach ($referers = $note->relativeOf as $referer)
             <section
                 class="relative min-h-full overflow-y-auto  @if ($loop->first && $referers->count() == 1) min-w-[100%] @endif @if (!$loop->last) min-w-[100%] lg:min-w-[50%] lg:border-r-4 border-slate-200 dark:border-slate-700 @else min-w-[100%] lg:min-w-[50%] @endif">
                 <div
